...
## Step 2 (Start Project/프로젝트 시작하기)
 ...
 ```django-admin startproject dinewithdjango```
...
After creating the project, we have to change `TIME_ZONE` in `settings.py` file. You can find it under `dinewithdjango/dinewithdjango/settings.py` folder. It depends on where your site is hosted. For Seoul, we will change it the following.
...
b. We are going to create a model for our menu app. From here onwards, start following me on and refer to tutorial for references. The required fields for our model class `Restaurant` are `name,ratings,price,image and createdAt`
...
class Cupcake(models.Model):
    ...
    def __str__(self):
        ...
        ```
 c. After creating model, we create actual tables in database by using `python manage.py makemigrations rests` and `python manage.py migrate`. 
 > Note that `makemigrations` command creates the script in `dinewithdjango/menu/migrations` folder which should be executed using `migrate command`
 

#### 데모 여기부터 

b. `menu` 글 모델 만들기 속성:
`name,ratings,price,image and createdAt` fields
